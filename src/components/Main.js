...
export default class Main extends Component {
    ...
    sourceClick(event) {
        let target = event.target.value !== 'null' ? {query: `everything?sources=${event.target.id}`}
                                                : {query: "top-headlines?country=ru&pageSize=5&page=1&"}
        ...
        return (
            <main className="app__main">
                <div className="app_search-block">
                    ...
                    <SourcesSelect sources={this.state.sources} clickHandler={this.sourceClick}/>
                </div>
